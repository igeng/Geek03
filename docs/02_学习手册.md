...
#### BochaSearch API（用于 Web 搜索，11.websearch 模块需要）

1. 访问 BochaAI 官网注册账号
2. 获取 API Key
3. 在 `11.websearch/.env` 中配置 `BOCHA_API_KEY`

> 💡 **初学者提示：** `18.deepresearch` 和 `12.websearch` 模块使用的是 SearXNG 搜索引擎（自部署搜索服务），不需要额外的 API Key，但需要可用的 SearXNG 服务地址。
...
langgraph            0.2.x
langchain-community  0.3.x（SearXNG 搜索工具）
langchain-mcp-adapters 0.1.x
streamlit            1.40.x
python-dotenv        1.0.x
openai               1.x（06.ReAct 模块使用原生 OpenAI SDK）
akshare             1.14.x（金融模块）
pandas              2.x（金融模块）
...
**`06.ReAct/react/tools.py`：工具定义（字典格式）**

```python
# 工具使用字典格式描述（非 LangChain @tool 装饰器）
tools = [
    {
...
> 💡 **初学者提示：** `06.ReAct` 模块的工具定义采用的是手动描述的字典格式，这与 OpenAI Function Calling 的原始格式一致。后续模块（如 `07.CodeAct`、`18.deepresearch`）才使用 LangChain 的 `@tool` 装饰器，更加简洁。

**`06.ReAct/react/agent.py`：手动实现 ReAct 循环**

```python
import json
from llm import client  # 通义千问 (qwen-max) 客户端
...
# 核心 ReAct 循环：不断发送消息→解析 Action→执行工具→返回 Observation
...
while True:
    ...
    if final_answer_match:
        print("最终答案:", final_answer_match.group(1))
        break
...
> ⚠️ **注意：** `06.ReAct/react/` 使用的是通义千问 (`qwen-max`)，不是 DeepSeek。它通过 `openai` SDK 连接阿里云 API（OpenAI 兼容接口）。而 `06.ReAct/functioncalling/` 使用的是 DeepSeek 原生的 Function Calling 功能。
...
**学习内容：**
- [ ] 确保 SearXNG 搜索服务可用（DeepResearch 依赖 SearXNG）
