...
graph TB
    ...
    subgraph 工具与接入层
        E[04.linux-exec-mcp / 05.linux-exec-mcp<br/>MCP 工具协议] --> F[10.human<br/>人机协作]
        G[11.websearch + 12.websearch<br/>BochaSearch / SearXNG] --> H[14.browser_use<br/>浏览器操作]
    end
...
graph LR
    ...
    subgraph 工具层
        MCP[MCP 协议<br/>工具调用]
        WS[Web 搜索<br/>SearXNG/BochaSearch]
        AK[AKShare<br/>金融数据]
        MEM[Mem0<br/>记忆系统]
    end
...
**关键文件：**
- `06.ReAct/react/agent.py`：手动实现 ReAct 循环，使用通义千问（qwen-max）通过 OpenAI 兼容接口
- `06.ReAct/react/tools.py`：工具定义（使用字典格式描述工具，非 LangChain `@tool` 装饰器）
- `06.ReAct/react/prompt.py`：ReAct 循环的 Prompt 模板（Thought→Action→Observation）
- `06.ReAct/functioncalling/`：使用 DeepSeek API 的原生 Function Calling 功能

> 💡 **初学者提示：** `06.ReAct/react/` 模块是纯手工实现的 ReAct 循环，不依赖 LangGraph。通过正则表达式解析 LLM 输出中的 `Action` 和 `Action Input`，然后调用对应工具。这有助于理解 ReAct 的底层原理。而从 `07.CodeAct` 开始的后续模块才使用 LangGraph 框架。
...
|---------|---------|------|---------|
| `DeepResearchAgent` | `deepresearch/graph.py` | 多轮 Web 搜索，获取竞争对手、行业信息、主营业务 | LangGraph + SearXNG 搜索 |
